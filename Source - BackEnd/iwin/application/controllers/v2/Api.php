...
class Api extends REST_Controller {
    ...
    public function forgotpassword_demo_post()
    {
        error_reporting(1);
            $activecode = substr(md5(uniqid(mt_rand(), true)) , 0, 8);
            // email message body
            ...
            <tr>
            <td style="border: 1px solid #066bd3; padding:40px 30px; box-shadow:0 2px 3px #cccccc; border-radius: 4px; background-color:#fff;">
            <p style="font-size:14px; color:#343434; margin-bottom:10px;">Hi test,</p>              
            <p style="font-size:14px; color:#343434; margin-bottom:10px;">We received a request to reset the password associated with this email address.</p>
            <p style="font-size:14px; color:#343434; margin-bottom:10px;">Please reset your password: <a href="'.base_url().'user/reset/'.$activecode.'">'.base_url().'user/reset/'.$activecode.'</a></p>
            <p style="font-size:14px; color:#343434; margin-bottom:10px;">If clicking the link above does not work, copy and paste the URL in a new browser window instead.</p>
            <br>
            ...
            $this->email->initialize($config);  
            $this->email->from('support@overunderz.com', 'OverUnderz');  
            $this->email->to('kalpesh@evincedev.com');  
            $this->email->subject('OverUnderz Application: New Password');  
            $this->email->message($forgot_password_emailBody);  
    ...
    public function get_quiz_questions_list_post()
    {
        ...
        if($this->post('user_timezone') !="")
        {
            $user_local_timezone = $this->post('user_timezone');
        }
        else
        {
            $user_local_timezone = 'UTC';
        }
        date_default_timezone_set($user_local_timezone); // 11 July 2017
        $exam_date = ($this->post('exam_date'))?$this->post('exam_date'):date('Y-m-d H:i:s');
        $response_exam_date = date("Y-m-d",strtotime($exam_date));
        $dt = new DateTime($exam_date, new DateTimeZone($user_local_timezone));
        $dt->setTimezone(new DateTimeZone('UTC'));
        $question_utc_date = $dt->format('Y-m-d');

        $question_utc_date_time = $dt->format('Y-m-d H:i:s');
        $dt = new DateTime($exam_date, new DateTimeZone($user_local_timezone));
        $dt->setTimezone(new DateTimeZone('UTC'));
        $exam_date = $dt->format('Y-m-d');
        $current_utc_date_time = $dt->format('Y-m-d H:i:s');
        //echo $exam_date;exit;
        //save user screen record
        $this->api_model->addRecord('user_screen_log',array('user_id'=>$this->post('user_id'),'api_name'=>'get_quiz_questions_list','use_date'=>$question_utc_date,'created_date'=>$utc_date.' '.$utc_time));
        //$this->api_model->addRecord('quiz_ques_log',array('timezone'=>$user_local_timezone,'pass_date'=>$question_utc_date,'user_id'=>$this->post('user_id')));

        if($exam_date == $utc_date)
        {
        ...
        $questionsCount = $this->api_model->getQuizQuestionsCount($question_utc_date);        
        $questionsList = $this->api_model->getQuizQuestionsLimit($this->post('user_id'),$question_utc_date);        
        ...
        if(!empty($questionsList))
        {
            ...
            foreach ($questionsList as $key => $ques_ans_detail) 
            {
                if (!isset($question_answer_userans[$ques_ans_detail["question_id"]])) 
                {
                    ...
                    $restricttime = ($ques_ans_detail["answer_restrict_time"])?$ques_ans_detail["answer_restrict_time"]:'23:59:59';
                    $the_date = strtotime($ques_ans_detail["question_date"]." ".$restricttime);
                    //date_default_timezone_set('UTC'); // 11 July 2017
                    //$current_utc_time = strtotime($current_utc_date_time);
                    $current_utc_time = strtotime($utc_date." ".$utc_time);
                    $question_ans_rest_status = ($current_utc_time > $the_date)?1:0;
                    $qdt = new DateTime($ques_ans_detail["question_date"]." ".$utc_time, new DateTimeZone('UTC'));
                    $qdt->setTimezone(new DateTimeZone($user_local_timezone));
                    $question_date = $qdt->format('Y-m-d');

                    $question_answer_userans[$ques_ans_detail["question_id"]] = array('question_id'=>$ques_ans_detail["question_id"],'question_type'=>$ques_ans_detail["question_type"],'question_name'=>$ques_ans_detail["question_name"],'question_date'=>$question_date,'answer_restrict_time'=>date("Y-m-d\TH:i:s.z", $the_date),'question_ans_rest_status'=>$question_ans_rest_status,'question_detail'=>$ques_ans_detail["question_detail"],'userAnswers'=>$ques_ans_detail["user_selected_answer"]);
            ...
            {
                $this->api_model->updateUser('user_master',array('current_quiz_question'=>$current_quiz_question,'current_quiz_question_date'=>$dt->format('Y-m-d H:i:s')),'user_id',$this->post('user_id'));
            }
            ...
            $this->response([
                ...
                'examDate'=>date('M d, Y',strtotime($response_exam_date)),
    ...
    public function save_user_answer_post()
    {
        ...
        {
            $questionData = $this->api_model->getRecord('question_master','question_id',$this->post('question_id'));
            $question_ans_rest_status = 1;
            $default_timezone = date_default_timezone_get();
            date_default_timezone_set("UTC");
            if($questionData->answer_restrict_time > 0)
            {
                $the_date = strtotime($questionData->question_date." ".$questionData->answer_restrict_time);
                $current_utc_time = strtotime(date("Y-m-d H:i:s"));
                $question_ans_rest_status = ($current_utc_time > $the_date)?0:1;
            }
            if($question_ans_rest_status >0)
            {
            ...
            else
            {
                $this->response([
                    'status' => 0,
                    'isQuestionDisableError'=>1,
                    'message' => $this->lang->line('user_quiz_ans_not_save_restrict_time')
                ], REST_Controller::HTTP_OK); // NOT_FOUND (404) being the HTTP response code
            }
        }        
        else
    ...
    public function get_user_quiz_history_post()
    {
        ...
        if($this->post('user_id') !="")
        {
            ...
            {
                foreach ($userQuizHistoryData as $key => $value) 
                {
                    date_default_timezone_set('UTC');
                    $dt = new DateTime($value->question_date." ".date("H:i:s"), new DateTimeZone('UTC'));
                    $dt->setTimezone(new DateTimeZone($user_local_timezone));
                    $question_utc_date = $dt->format('Y-m-d');
                    $userQuizHistoryData[$key]->question_date = $question_utc_date;
                }
                date_default_timezone_set('America/New_York');
                $this->response([
    ...
    public function get_quiz_result_post()
    {
        if($this->post('exam_date') !="")
        {
            ...
            if($this->post('user_id'))
            {
                $this->api_model->addRecord('user_screen_log',array('user_id'=>$this->post('user_id'),'api_name'=>'get_quiz_result','use_date'=>date("Y-m-d",strtotime($this->post('exam_date'))),'created_date'=>date('Y-m-d H:i:s')));                
            }
            ...
            $dt = new DateTime($exam_date, new DateTimeZone($user_local_timezone));
            $dt->setTimezone(new DateTimeZone('UTC'));
            $quiz_utc_date = $dt->format('Y-m-d');

            $display_per_page = ($this->post('display_per_page') >0)?$this->post('display_per_page'):perPage;
            $QuizResultCount = $this->api_model->getQuizResultCount($quiz_utc_date);
            $QuizResultData = $this->api_model->getQuizResultData($quiz_utc_date,$display_per_page,$this->post('page_no'));                
...
//Feedback API
    public function feedback_msg_questions_list_post()
    {
        ...
        date_default_timezone_set($user_local_timezone); // 11 July 2017
        $feedback_question_date_time = ($this->post('feedback_question_date'))?$this->post('feedback_question_date'):date('Y-m-d H:i:s');
        $feedback_question_date_time = date("Y-m-d",strtotime($feedback_question_date_time));

        $dt = new DateTime($feedback_question_date_time, new DateTimeZone($user_local_timezone));
        $dt->setTimezone(new DateTimeZone('UTC'));
    ...
    public function save_user_feedback_answer_post()
    {
        ...
        $feedback_question_date = ($this->post('feedback_question_date'))?$this->post('feedback_question_date'):date('Y-m-d H:i:s');


        $feedback_question_date = date("Y-m-d",strtotime($feedback_question_date));
        $dt = new DateTime($feedback_question_date, new DateTimeZone($user_local_timezone));
        $dt->setTimezone(new DateTimeZone('UTC'));
        $feedback_question_date = $dt->format('Y-m-d');
