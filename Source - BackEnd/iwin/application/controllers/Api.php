...
class Api extends REST_Controller {
    ...
    public function forgotpassword_post()
    {
        ...
        if(!empty($checkRecord))
        {
            ...
            <img src="'.base_url().'assets/images/logo.jpg" alt="iWin App Logo" style="-ms-interpolation-mode:bicubic; width:220px; padding-bottom:10px;"><br>              
            </td>
            </tr>
            </table>
            ...
            <p style="font-size:14px; color:#343434; margin-bottom:10px;">New Password: '.$NewPassword.'</p>';
            "</td>                         
            </tr>
            </table>            
            </table>
            ...
            $this->email->from('kalpesh@evincedev.com', 'iWin');  
            $this->email->to(trim($this->input->post('email')));  
            $this->email->subject('iWin Application: New Password');  
    ...
    public function get_quiz_questions_list_post()
    {
        ...
        if(!empty($questionsList))
        {
            ...
            {
                $this->api_model->updateUser('user_master',array('current_quiz_question'=>$current_quiz_question),'user_id',$this->post('user_id'));
            }

            //get time for ads display setting
            $timeForAds = 3;
            $timeForAdsData = $this->api_model->getRecord('settings','setting_name','timeForAds');
            if(!empty($timeForAdsData))
            {
                $timeForAds = ($timeForAdsData->setting_value !="" && $timeForAdsData->setting_value >0)?$timeForAdsData->setting_value:3;
            }
            $this->response([
    ...
    public function save_user_answer_post()
    {
        if($this->post('user_id') !="" && $this->post('question_id') !="")
        {
            ...
            $this->api_model->updateUser('user_master',array('current_quiz_question'=>$this->post('current_quiz_question')?$this->post('current_quiz_question'):null),'user_id',$this->post('user_id'));                
        ...
        if($this->post('exam_date') !="")
        {
            ...
            else
            {
                $this->response([
                    ...
                    'user_quiz_history_data'=>$QuizResultData,
                    'status' => 1,
...
//Feedback API
    public function feedback_msg_questions_list_post()
    {
        ...
        $feedback_questions_list = $this->api_model->getFeedbackQuestionList();        
    ...
    {
        if($this->post('user_id') !="" && $this->post('feedback_answers') !="")
        {
            $userAnsArray = json_decode($this->post('feedback_answers'), true);
            ...
            {
                $this->api_model->deleteInsertRecord('feedback_user_answer',$save_user_feedback_ans,array('user_id'=>$this->post('user_id'),'feedback_question_date'=>$this->post('feedback_question_date')));                
            }
            $this->response([
                'status' => 1,
                'message' => $this->lang->line('user_quiz_ans_save')], REST_Controller::HTTP_OK); // OK (200) being the HTTP response code            
