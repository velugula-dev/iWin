                ...
                <div class="col-md-12">
                    ...
                    <div class="portlet box red">
                        <div class="portlet-title">
                            <div class="caption">Question List</div>
                            <div class="actions">
                                <a class="btn danger-btn btn-sm" href="<?php echo config_item('admin_base_url');?>question/add"><i class="fa fa-plus"></i> Add</a>
                            </div>
                        </div>
                        <div class="portlet-body">
                            <div class="table-container">
                            <?php 
                            if($this->session->flashdata('PageMSG'))
                            {?>
                                <div class="alert alert-success">
                                    <strong>Success!</strong> <?php echo $this->session->flashdata('PageMSG');?>
                                </div>
                            <?php } ?>
                            <div id="delete-msg" class="alert alert-success hidden">
                                ...
                                <table class="table table-striped table-bordered table-hover" id="datatable_ajax">
                                        ...
                                        <tr role="row" class="filter">
                                            <td></td>                                       
                                            <td><input type="text" class="form-control form-filter input-sm" name="question_name_search"></td>
                                            <td>
                                                <select name="question_type_search" class="form-control form-filter input-sm">
                                            ...
                                            <td><input id="question_date_search" type="text" class="form-control form-filter input-sm" readonly name="question_date_search" value=""></td>
                                            <td>
                                                <div class="input-group">
                                                    <input type="text" class="form-control form-filter timepicker timepicker-24" name="answer_restrict_time" id="answer_restrict_time" value="">
                                                    <span class="input-group-btn">
                                                    <button class="btn default" type="button"><i class="fa fa-clock-o"></i></button>
                                                    </span>
                                                </div>
                                            </td>
                                            <td><input type="text" class="form-control form-filter input-sm" name="question_detail_search"></td>
                                            <td>
...
$('#question_date_search').datepicker({               
        autoclose:true,
        format: 'yyyy-mm-dd'
});    
...
function disableRecord(question_id,status)
{
    var statusVar = (status==0)?'Active':'Deactive';
    bootbox.confirm("Are you sure wants to "+statusVar+" question?", function(disableConfirm) {    
        if (disableConfirm) {
            jQuery.ajax({
              type : "POST",
              dataType : "json",
              url : BASEURL+'question/ajaxDisable',
...
{
    bootbox.confirm("Are you sure wants to delete question?", function(disableConfirm) {    
        if (disableConfirm) {
            jQuery.ajax({
              type : "POST",
              dataType : "json",
              url : BASEURL+'question/check_question_attempt_user',
              data : {'question_id':question_id},
              success: function(response) {
                if(response !="" && response > 0)
                {
                   bootbox.alert("Can not delete this question as it has been attempted by the users."); 
                }
                else
                {
                    jQuery.ajax({
